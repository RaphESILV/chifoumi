...
function Game() {
  ...
  try {
    const response = await axios.post(`${API_URL}/matches`, endGameData, {
        headers: {
    ...
    if (response.status === 200) {
      console.log('End game data saved successfully:', response.data);
    } else {
      console.error('Failed to save end game data:', response.data);
    }
  } catch (error) {
    console.error('An error occurred while saving the end game data:', error);
  }
};
